...
export default function Variant({
  ...
  const handleChangeParentDebounced = useCallback(
    debounce((parent) => {
  ...
  const handleChangeParent = async (value: string) => {
    ...
    try {
      const { data } = await axios.post('https://datn-backend-production-abfd.up.railway.app/api/option', {
        name: value
      })
  ...
  const handleAddFieldDebounced = useCallback(
    debounce((value, index, id) => {
      if (index === inputField) {
        const newFields = detail.map((field) => {
          if (field.id === keyValue) {
            return {
  ...
  return (
    <Flex vertical gap={10} className='sm:rounded-xl overflow-hidden relative p-5 border-[1px]'>
      <Flex
        justify='center'
        align='center'
        className='p-1 rounded-md border-[1px] absolute right-2 top-2 cursor-pointer'
      >
        <MinusOutlined
          className=' '
          onClick={() => {
            handleRemoveDetail(keyValue)
          }}
        />
      </Flex>
      <div>
              ...
              {index < 5 ? (
                ...
                >
                  <Flex className='relative w-full' align='center' gap={10}>
                    {show === 0 ? (
                      ...
                      >
                        {value.url ? (
                          ...
                          <Flex
                            className='border-dashed border-2 relative hover:bg-gray-100 bg-white hover:border-solid hover:border'
                            vertical
                            gap={10}
                            justify='center'
                            align='center'
                    ...
                    {index < inputField - 1 ? (
                      <div
                        className='w-[25px] h-[25px] rounded-full bg-[#fff] absolute top-[-10px] right-[-10px] flex items-center justify-center hover:text-blue-500 cursor-pointer overflow-hidden'
