...
export default function Option({ setImageUrl, setCategory, thumbnail }: option) {
  ...
  const brands = dataBrands
    ? dataBrands?.data?.map((item: IBrand) => {
        return {
          label: item.name,
          value: item.id
        }
  ...
  const getDetails = async (value: string) => {
    const { data } = await axios.get(`https://datn-backend-production-abfd.up.railway.app/api/category/show/${value}`)
    ...
    <Flex vertical gap={30}>
      ...
      >
        <Flex vertical gap={20}>
          ...
          <div
            style={{ height: '10.5vw', boxShadow: '0 0.5rem 1.5rem 0.5rem rgba(0, 0, 0, 0.075)' }}
            className='border-none rounded-xl relative '
          >
            {DisplayPic ?? thumbnail ? (
              <div
                style={{ height: '100%', maxWidth: '100%', overflow: 'hidden' }}
                className='relative group border-none rounded-xl'
              >
                <img
                  src={DisplayPic ?? thumbnail}
                  alt=''
              ...
              <Flex
                ...
                justify='center'
                align='center'
                style={{ maxWidth: '100%', height: '100%', borderRadius: '12px', overflow: 'hidden' }}
              >
                <Flex vertical gap={10} style={{ width: '100%' }}>
                  <Flex vertical align='center' justify='center'>
                    <PermMediaRoundedIcon
                      style={{ fontSize: '60px', color: 'rgb(31 41 55 / var(--tw-text-opacity))' }}
                      className=''
                    />
                  </Flex>
                </Flex>
      ...
      {/* category */}
      <div
        className='sm:rounded-lg flex-1 p-2 relative bg-[#ffff]'
        style={{ boxShadow: 'rgba(0, 0, 0, 0.05) 0rem 1.25rem 1.6875rem 0rem' }}
      >
        <div className='p-2'>
        ...
        <Flex justify='center' align='' vertical className='p-2' gap={10}>
          ...
          <Flex
            align='center'
            justify='center'
            className='w-[30px] h-[30px] cursor-pointer rounded-[9999px] absolute top-[-10px] right-[-9px] bg-[#fff]'
            style={{ boxShadow: '0 0.5rem 1.5rem 0.5rem rgba(0, 0, 0, 0.075)' }}
          >
            <PlusOutlined />
          </Flex>
      ...
      >
        ...
        <Flex justify='center' align='' vertical className='p-2' gap={10}>
          <Form.Item className='m-0' name='tags'>
            <Select className='custom-seclect' placeholder={'Chọn từ khóa'} mode='tags' style={{ width: '100%' }} />
          </Form.Item>
        </Flex>
      </div>
      {/* Tags */}
      ...
      {/* Setting */}
      <div
        className='sm:rounded-lg overflow-hidden flex-1 p-2 bg-[#ffff]'
        style={{ boxShadow: 'rgba(0, 0, 0, 0.05) 0rem 1.25rem 1.6875rem 0rem' }}
      >
        <div className='p-2'>
        ...
        <div className='flex justify-between items-center p-2'>
          ...
          <Form.Item className='m-0' label='' name='is_good_deal' valuePropName='checked'>
            <Switch />
          </Form.Item>
        </div>
        <div className='flex justify-between items-center p-2'>
        ...
        <div className='flex justify-between items-center p-2'>
          ...
          <Form.Item className='m-0' label='' name='is_show_home' valuePropName='checked'>
            <Switch />
          </Form.Item>
        </div>
      </div>
